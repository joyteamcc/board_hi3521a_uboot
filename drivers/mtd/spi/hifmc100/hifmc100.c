	...
	const void *buf)
{
	...
	if (host->level && (to < host->end_addr)) {
		puts("\n");
		DB_MSG("ERROR: The DMA write area was locked\n");
		return -EINVAL;
	}
#endif
	...
	size_t length)
{
	...
	if ((host->level) && (offset < host->end_addr)) {
		puts("\n");
		DB_MSG("ERROR: The erase area was locked\n");
		return -EINVAL;
	}
#endif
...
struct spi_flash *hifmc100_spi_nor_scan(struct hifmc_host *host)
{
	...
	hifmc100_get_spi_lock_info(host, &(host->level));
...
void spi_lock_update_address(struct hifmc_host *host)
{
	...
	unsigned char id_manu = host->spi_nor_info->ids[0];
	struct hifmc_spi *spi = host->spi;
		...
		unsigned char *level)
{
	...
	case ID_MANU_W:
		...
		if (spi->chipsize <= _16M) {
			*level = (val & SPI_NOR_SR_BP_MASK_3)
				>> SPI_NOR_SR_BP0_SHIFT;
		} else
			*level = (val & SPI_NOR_SR_BP_MASK_4)
				>> SPI_NOR_SR_BP0_SHIFT;
		break;
	case ID_MANU_S:
	...
	case ID_MANU_MX:
		if (spi->chipsize >= _16M) {
			...
			*level = (val & SPI_NOR_SR_BP_MASK_4)
			...
			val = hifmc100_handle_bp_rdsr_info(host,
					SPI_CMD_RDSR, BP_NUM_3);
			*level = (val & SPI_NOR_SR_BP_MASK_3)
				>> SPI_NOR_SR_BP0_SHIFT;
		}
		break;
	...
	if (host->cmp == BP_CMP_UPDATE_FLAG) {
		...
		FMC_PR(BP_DBG, "Spi is locked. lock address[0 => %#x]\n",
				host->end_addr);
	}
	return val;
usage:
...
/*****************************************************************************/
unsigned short hifmc100_handle_bp_rdsr_info(struct hifmc_host *host, u_char cmd,
		unsigned int bp_num)
{
	unsigned char val;
	struct hifmc_spi *spi = host->spi;
	...
	FMC_PR(BP_DBG, "Get Status Register[%#x]\n", val);
	if (id_manu == ID_MANU_EN) {
		bp_num = BP_NUM_3;
		val &= SPI_BP_BOTTOM_RDSR_SET_0(bp_num);
	} else if ((id_manu == ID_MANU_W) && (spi->chipsize <= _16M)) {
			bp_num = BP_NUM_3;
			val |= SPI_BP_BOTTOM_RDSR_SET_1(bp_num);
		}
	else
		val |= SPI_BP_BOTTOM_RDSR_SET_1(bp_num);
	host->bp_num = bp_num;
...
static void hifmc100_set_BP_level(struct hifmc_host *host, unsigned char level)
{
	...
	val = hifmc100_get_spi_lock_info(host, &old_level);
	...
	if (((id_manu == ID_MANU_F) || ((id_manu == ID_MANU_MX)
...
void hifmc100_spi_lock(struct hifmc_host *host, unsigned char level)
{
	...
	current_level = hifmc100_bp_to_level_check(host);
...
/*****************************************************************************/
unsigned char hifmc100_bp_to_level_check(struct hifmc_host *host)
{
	unsigned char val;
	unsigned char level;
	struct hifmc_spi *spi = host->spi;
